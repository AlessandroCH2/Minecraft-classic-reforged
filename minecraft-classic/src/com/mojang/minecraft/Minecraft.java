...
import com.alessandroch.minecraft.EnglishLanguage;
import com.alessandroch.minecraft.ItalianLanguage;
import com.alessandroch.minecraft.Language;
...
public final class Minecraft implements Runnable {
   ...
   public Language language = new EnglishLanguage("English");
   public static Minecraft instance;
   private boolean fullscreen = false;
   public String version = "1.0.0rf";
   public int width;
   public int height;
   public float topbartime = 0;
   public String textTopBar = "None";
...
return language;	
   ...
   public Minecraft(Canvas var1, MinecraftApplet var2, int var3, int var4, boolean var5, String username, String password, String serverIp) {
      ...
      this.selected = null;
      this.server = serverIp.split(":")[0];
     if(!serverIp.equals("none")) session = new SessionData(username,""+new Random().nextInt(5000));
      this.port = 25565;
      if(serverIp.split(":").length > 1) this.port = Integer.parseInt(serverIp.split(":")[1]);
      this.running = false;
      this.debug = "";
      this.hasMouse = false;
      this.lastClick = 0;
...
instance = this;
   ...
   public final void run() {
      ...
      try {
         ...
         if(this.canvas != null) {
            ...
            Display.setDisplayMode(new DisplayMode(canvas.WIDTH, canvas.HEIGHT));
        ...
        if(f.exists() == false) f.mkdir();
         this.settings = new GameSettings(this, f);
         ...
         //Connect into the Reforged account
         ConnectToAccount();
                        ...
                        if(var66.minecraft.level != null) {
                           ...
                           while(true) {
                              ...
                              /*if(var82.entity != null) {
                               ...
                               for(int i = 0; i < var82.minecraft.level.getEntities().all.size(); i++) 
                               {
                            	   
                            	   Object plr = var82.minecraft.level.getEntities().all.get(i);
                            		   ((Entity) plr).renderHover(var82.minecraft.textureManager, var80);
   ...
   private void ConnectToAccount() {
	// TODO Auto-generated method stub
	 ...
	 //  setCurrentScreen(new ErrorScreen("Authentication", "Unable to login into your account"));
	   // return;
}
...
private boolean spawned;
   ...
   private void tick() {
                     ...
                     while(var22.in.position() > 0 && var4++ != 100) {
                        ...
                        if(var22.netManager.successful) {
                           if(var6 == PacketType.IDENTIFICATION) {
                             ...
                             // this.networkManager.sendPosition(this.player.x, this.player.y, this.player.z, this.player.xRot, this.player.yRot);
                              ...
                              if(var6 == PacketType.SPAWN_PLAYER) {
                                 ...
                                 if(var5 >= 0) {
                                    ...
                                    if(var7[0].equals(session.username)) 
                                    {
                                  ...
                                  if(alreadySpawned == false)  var42.minecraft.player.moveTo((float)var36 / 32.0F, (float)var47 / 32.0F, (float)var10 / 32.0F, (float)(var9 * 360) / 256.0F, (float)(var58 * 360) / 256.0F);
                                  if(alreadySpawned == false )  alreadySpawned = true;
                                 }
                              } else {
                                 byte var53;
                                 NetworkPlayer var61;
                                 ...
                                 if(var6 == PacketType.POSITION_ROTATION) {
                                    ...
                                    short var38 = var66;
                                    var5 = var10001;
                                    float x = (float) ((Short)var7[1]).doubleValue();
                                    float y = (float) ((Short)var7[2]).doubleValue();
                                    float z = (float) ((Short)var7[3]).doubleValue();
                                    if(var5 < 0) {
           
                                     // var42.minecraft.player.moveTo((float)x/100, (float)y/100, (float)z/100, (float)(var53 * 360) / 256.0F, (float)(var9 * 360) / 256.0F);
                                      // 
                                    } else {
                                    ...
                                    } else if(var6 == PacketType.PLAYER_TELEPORT) 
                                    {
                                    	var10001 = ((Byte)var7[0]).byteValue();
                                        ...
                                        float y = (float) ((Short)var7[2]).doubleValue();
                                        float z = (float) ((Short)var7[3]).doubleValue();
                                        var42.minecraft.player.setPos(x, y, z);
                                    
                                    }else if(var6 == PacketType.GET_SCOREBOARD) 
                                    {
                                    	
                                        String scoreName = ((String) var7[0]);
                                      
                                       int length = ((int) var7[1]);
                                       String[] scores = new String[length];
                                       for(int i = 0; i > length; i++) 
                                       {
                                    	   scores[i] = "Nothing in this score";
                                       }
                                        scoreboard = new Scoreboard(scoreName,scores);
                                    }else if(var6 == PacketType.SET_SCORE) 
                                    {
                                       ...
                                       scoreboard.scores.set(length, new Score(scoreName));
                                    }else if(var6 == PacketType.SET_HOTBARTEXT) 
                                    {
                                       ...
                                       this.setTopBarText(scoreName);
                                    }else if(var6 == PacketType.JOINNEWSERVER) 
                                    {
                                    	
                                        String scoreName = ((String) var7[0]);
                                      
                                    
                                       String[] split = scoreName.split(":");
                                       if(split.length > 1) 
                                       {
                                    	   connectToOtherServer(split[0], Integer.parseInt(split[1])); 
                                       }else 
                                       {
                                    	   connectToOtherServer(split[0], 25565); 
                                       }
            ...
            if(Keyboard.getEventKeyState()) {
               ...
               for(var25 = 0; var25 < 9; ++var25) {
                  if(Keyboard.getEventKey() == var25 + 2) {
                     ...
                     this.topbartime = 5;
                     this.textTopBar = ""+Block.getBlockType(this.player.inventory.slots[this.player.inventory.selected]).id;
                  }
               }
      ...
      if(this.level != null) {
         ...
         if(topbartime > 0)topbartime-=0.1f;
 ...
 //=========================getScreenImage==================================//
   public void saveScreenshot() throws Exception {
	   ...
	   int[] pixels = new int[WIDTH * HEIGHT];
	    ...
	    try {//Try to screate image, else show exception.
	        ...
	        File output1 = new File("screenshots/"+formatter.format(date)+".png"); 
   ...
   public void setTopBarText(String txt) 
   {
	   topbartime = 5;
	textTopBar = txt; 
   }
   public void connectToOtherServer(String address, int port) 
   {
       Minecraft var1 = this;
         if(this.networkManager != null) this.networkManager.netHandler.close();
	   if(this.server != null && this.session != null) {
           Level var85;
           (var85 = new Level()).setData(8, 8, 8, new byte[512]);
           this.setLevel(var85);
        ...
        (new SkinDownloadThread(this)).start();
        if(this.server != null && this.session != null) {
           this.networkManager = new NetworkManager(this, address, port, this.session.username, this.session.mppass);
        }
   }
}
